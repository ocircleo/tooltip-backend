...
function JwtVerifyFunction(req, res, next) {
  ...
  if (!token) {
    return res
      .status(404)
      .send({ error: true, message: "No Token" });
  }
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (error, decoded) => {
    if (error) {
      return res
        .status(404)
        .send({ error: true, message: "False Token" });
    }
    req.decoded = decoded;
    next();
  });
...
AuthRoute.put("/auto_login", JwtVerifyFunction, async (req, res) => {
  ...
  try {
    ...
    if (!result)
      return res.send(ReturnMessage(true, "No user found by this email"));
    //checks the password
    ...
    res
      ...
      .send(ReturnMessage(false, "Login Successful", result));
  } catch (error) {
...
async function AuthorizeUser(req, res, next) {
  ...
  if (result.password != key) return res.status(404).send(ReturnMessage("Password not matched", 404))
    req.user = result;
    next();
}
